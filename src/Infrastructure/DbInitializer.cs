            ...
            using (var context = serviceScope.ServiceProvider.GetService<OrderDbContext>())
            {
                if (context != null)
                {
                    ...
                    if (!context.Orders.Any())
                    {
                        ...
                        context.Add(new OrderItemMap() { OrderId = ordr.Entity.Id, ItemId = itm1.Entity.Id, Qty= 14 });
                        context.Add(new OrderItemMap() { OrderId = ordr.Entity.Id, ItemId = itm2.Entity.Id, Qty = 11 });
                        context.SaveChanges();
                    }
                }
            }
